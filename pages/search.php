<?php
    session_start();
    $str_search = filter_input(INPUT_GET, 'q', FILTER_SANITIZE_STRING);
    //$str_search = strtoupper($str_search);
    $int_search = (int)$str_search;
    ...
    $regex = new MongoDB\BSON\Regex($str_search, 'i');
    //Create a PHP array with our search criteria
    $findCriteria = ['$or' => [['type' => $regex],['product_name' => $regex],['pid' => $int_search],['product_price' => $int_search],['product_description' => $regex],]];
    //Find all of the products that match  this criteria

    $list = $collection->find(
      $findCriteria,
      ['sort' => ['product_price' => 1]] //finds the product with the pid
      //[['type' => $str_search],['product_name' => $str_search],['pid' => $int_search],], ['sort' => ['product_price' => 1]] //finds the product with the pid
    );
?>
<!DOCTYPE html>
<html>
...
<head>
  ...
  <title><?php echo $str_search?></title>
...
<body>
  <h2>Results for <?php echo $str_search?></h2>
  ...
  <?php
    foreach ($list as $document) {
      ...
      if(isset($_SESSION['array'])){
        $_SESSION['array'][$document["pid"]]++;//increments for recommended products
      }
